...
pub(crate) struct Ipv4AddrPair<S: Endpoint> {
    pub src: Ipv4Addr,
    pub dst: Ipv4Addr,
...
impl<S: Endpoint> Ipv4AddrPair<S> {
    pub(crate) const fn new(src: Ipv4Addr, dst: Ipv4Addr) -> Self {
        Self { src, dst, phantom: PhantomData }
    }

    pub(crate) const fn swapped(self) -> Ipv4AddrPair<S::Peer> {
...
#[derive(Clone, Copy)]
#[cfg_attr(any(test, feature = "test-utils"), derive(Debug, PartialEq, Eq))]
pub(crate) struct PortPair<S: Endpoint> {
    pub src: u16,
    pub dst: u16,
    phantom: PhantomData<S>,
}
...
impl<S: Endpoint> PortPair<S> {
    pub(crate) const fn new(src: u16, dst: u16) -> Self { Self { src, dst, phantom: PhantomData } }

    pub(crate) const fn swapped(self) -> PortPair<S::Peer> {
