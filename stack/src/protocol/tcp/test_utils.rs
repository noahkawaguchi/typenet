...
pub(super) const CLIENT_ISN: SeqPoint<Remote> = SeqPoint::new(100);

/// Fixed value to use as the ISN randomly chosen by the server.
pub(super) const SERVER_ISN: SeqPoint<Local> = SeqPoint::new(400);

/// Connection key shared by test modules.
pub(super) const KEY: ConnKey = ConnKey {
...
pub(super) const WINDOW_AFTER_HANDSHAKE: WindowState = WindowState::test_new(
...
pub(super) const CLIENT_PKT: TcpSegment<Remote> = TcpSegment {
...
/// `seq_num` and `ack_num` will be 0 if not overridden.
pub(super) const SERVER_REPLY: TcpSegment<Local> = TcpSegment {
...
mod crate_only {
    ...
    pub(in super::super) const LOCAL_HELLO_LEN: SeqOffset<u32, Local> = SeqOffset::new(5);

    /// The number of bytes in the payload `"Hi"`, going in the local to remote direction.
    pub(in super::super) const LOCAL_HI_LEN: SeqOffset<u32, Local> = SeqOffset::new(2);

    /// The number of bytes in the payload `"Hey"`, going in the local to remote direction.
    pub(in super::super) const LOCAL_HEY_LEN: SeqOffset<u32, Local> = SeqOffset::new(3);

    /// The number of bytes in the payload `"Hello"`, going in the remote to local direction.
    pub(in super::super) const REMOTE_HELLO_LEN: SeqOffset<u32, Remote> = SeqOffset::new(5);

    /// The number of bytes in the payload `"Hi"`, going in the remote to local direction.
    pub(in super::super) const REMOTE_HI_LEN: SeqOffset<u32, Remote> = SeqOffset::new(2);

    /// The number of bytes in the payload `"Hey"`, going in the remote to local direction.
    pub(in super::super) const REMOTE_HEY_LEN: SeqOffset<u32, Remote> = SeqOffset::new(3);
}
